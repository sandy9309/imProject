...
- 掃描房間與手動牆是互斥的邊界來源。手動牆除了 Collider，也使用牆角多邊形判斷房內外；重建牆面後既有家具會依新的 geometry version 重新驗證。
- 掃描房間忽略 Meta 用於開放空間概念分區的 `INVISIBLE_WALL_FACE`；實體外牆與代表柱體等結構的 `INNER_WALL_FACE` 仍會阻擋家具。
- 已擺放家具使用 `placement-local-v1` 房間相對座標保存；舊 `world-v0` 世界座標於再次成功儲存時遷移。
- 搖桿或手把的目標被牆阻擋時，累積偏移會同步到實際合法位置，反向移動可立即離開牆面。
- 返回專案 ID 選擇時會先等待最後一次位置儲存，再停用並清除目前專案的虛擬家具。重新進入專案會自動還原所有 `isPlaced=true` 的家具；尚未擺放的品項仍留在清單供使用者選取。
- 掃描地板只參與家具高度校正，不加入程式的水平移動阻擋清單，避免傾斜或重複的地板錨點形成空地透明牆。
